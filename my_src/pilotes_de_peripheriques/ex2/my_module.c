...
#define NUMBER_OF_PERIPHS 1

//variable globale
char module_data[100];
...
static ssize_t my_read(struct file *f, char __user *buf, size_t count, loff_t *off){
    ...
    int len = count < 100 ? count : 100;
    // copie dans espace utilisateur
    // copy_to_user (void __user * to, const void * from, unsigned long n)
    if(copy_to_user(buf, module_data, len)){
...
static int __init my_module_init(void){
    ...
    if(cdev_add(&my_cdev, my_dev_buff, NUMBER_OF_PERIPHS)){
